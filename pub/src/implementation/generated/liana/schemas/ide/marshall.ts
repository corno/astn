...
export const Text_Edits: t_signatures.Text_Edits = ($,) => ['list', $.__l_map(($,) => ['state', _p.decide.state($, ($,): t_out.Value.state => {
    switch ($[0]) {
        case 'insert':
            return _p.ss($, ($,) => ({
                'option': 'insert',
...
export const ID_Value_Pairs_To_Be_Sorted: t_signatures.ID_Value_Pairs_To_Be_Sorted = ($,) => ['dictionary', $.__d_map(($,id,) => ['text', ({
    'delimiter': ['quote', null],
    'value': $,
})])]
